...
<script lang="ts">
  ...
  const lastVisibleRow = $derived(Math.min(currentPage * pageSize, totalRows));
...
`currentPage`, `pageCount`, `canPreviousPage`, `canNextPage`, `onPrevious`, `onNext`, `onGoToPage`, `siblingCount`, `firstVisibleRow`, `lastVisibleRow`, `totalRows`, `class`
